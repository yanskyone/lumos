...
| 项目 | 地址 |
|------|------|
| **项目首页** | https://supabase.com/dashboard |
| **项目地址** | https://supabase.com/dashboard/project/dmxytyvleoodvaggvkdg |

### 直接访问

```
https://dmxytyvleoodvaggvkdg.supabase.co
```
...
| 端点 | 地址 |
|------|------|
| **REST API** | `https://dmxytyvleoodvaggvkdg.supabase.co/rest/v1` |
| **Auth** | `https://dmxytyvleoodvaggvkdg.supabase.co/auth/v1` |
| **Storage** | `https://dmxytyvleoodvaggvkdg.supabase.co/storage/v1` |

---

## 🔑 连接配置

### 前端配置（cloud-storage.js）
...
> ⚠️ **注意**: 此为公开 API Key，仅用于客户端访问。如需保护，请使用 Row Level Security (RLS)。
...
| `mode` | VARCHAR(20) | 训练模式 | `flash_war` / `confusion` |
...
3. 写入策略
   ├─ 公共错题 → 写入 Supabase
   └─ 私有错题 → 写入 localStorage + Supabase（可选）
```
...
| 问题 | 解决方案 |
|------|---------|
| 400 错误 | 检查 RLS 策略配置是否正确 |
| CORS 错误 | 在 Supabase Settings -> API 添加允许的域名 |
| 数据不同步 | 检查网络连接，尝试刷新页面 |
| 离线模式异常 | 清除浏览器缓存，重新加载 |

### 本地调试

由于支持**离线优先模式**，本地调试不需要网络连接：
1. 断开网络或打开飞行模式
2. 访问 `file:///path/to/lumos/vocab/index.html`
3. 系统会自动使用 `data/errors.json` 作为数据源
